...
(async () => {
  for (const file of TARGETS) {
    ...
    try {
      ...
      const small = await img.resize(200).removeAlpha().raw().toBuffer({ resolveWithObject: false });
      // compute background color as average of corners of the small resized image
      const smallImg = sharp(input).resize(200).ensureAlpha().raw();
      const smallBufObj = await smallImg.toBuffer({ resolveWithObject: true });
      ...
      // sample a 10x10 area in each corner for a more robust background color
      ...
      console.log(file, 'bg color approx:', avg);

      // Create mask: make pixels close to bg color transparent
      // Use a larger threshold for more aggressive removal; tune if needed
      const threshold = 120; // increased from 70
